...
// export function validateRecord(record: SourceOrderData): void {
//   const errors: string[] = [];
//   const allowedStatuses = ['NEW', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'CANCELLED'];
//   if (!record || typeof record !== 'object') {
//     errors.push("Record must be a valid object.");
//     throw new Error(errors.join('\n')); // No need to continue if not an object
//   }
//   if (!record.orderId || typeof record.orderId !== 'string') {
//     errors.push("Invalid or missing 'orderId'.");
//   }
//   if (!record.orderDate || typeof record.orderDate !== 'string' || !/^\d{2}\/\d{2}\/\d{4}$/.test(record.orderDate)) {
//     errors.push("Invalid or missing 'orderDate'. Expected format: MM/DD/YYYY.");
//   }
//   if (!record.customerId || typeof record.customerId !== 'string') {
//     errors.push("Invalid or missing 'customerId'.");
...
//     errors.push("Invalid or missing 'totalAmount'. Must be a non-negative number.");
//   }
//   if (!record.status || typeof record.status !== 'string' || !allowedStatuses.includes(record.status)) {
//     errors.push(`Invalid or missing 'status'. Must be one of: ${allowedStatuses.join(', ')}.`);
//   }
//   if (record.shippingAddress) {
//     const addr = record.shippingAddress;
//     ['street', 'city', 'state', 'zipCode', 'country'].forEach(field => {
